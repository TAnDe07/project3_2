...
private:
    ...
    vector<string> helperMapSearchGenre(Node* helperRoot, string first) {
        Node* temp = helperMapSearch(helperRoot, first);
        vector<string> genres;
        vector<string> recommended;
        if (temp != nullptr) {
            inorderHelperGenre(temp, recommended, temp->value.genre);
        }
    ...
    vector<string>helperMapSearchYear(Node* helperRoot, string first) {
        Node* temp = helperMapSearch(helperRoot, first);
        int year;
        vector<string> recommended;
        if (temp != nullptr) {
    ...
    void inorderHelperGenre(Node* helperRoot, vector<string>& movies, vector<string>& genre) {
        //if tree is empty
        if (movies.size() == 5) {
            return;
        }
        ...
        for (int i = 0; i < helperRoot->value.genre.size(); ++i) {
            for (int j = 0; j < genre.size(); ++j) {
                if (helperRoot->value.genre.at(i) == genre.at(j)) {
                    movies.push_back(helperRoot->key);
                    break;
                }
            }
        }
    ...
    void inorderHelperYear(Node* helperRoot, vector<string>& movies, int year) {
        //if tree is empty
        if (movies.size() == 5) {
            return;
        }
...
public:
    ...
    vector<string> mapSearch(int selection, string first) {
        vector <string> recommended;
        //genre
        if (selection == 1) {
            recommended = helperMapSearchGenre(this->root, first);
            for (auto it = recommended.begin(); it != recommended.end(); it++) {
                cout << *it << endl;
            }
        }
        //year
